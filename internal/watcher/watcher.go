...
import (
	...
	"github.com/ponthepmk/AgentOrchestra/internal/notify"
...
type Config struct {
	...
	Notify      bool          // send a desktop notification on each auto-handoff
}
...
func (w *Watcher) matches(name string) bool {
	// HANDOFF.md is generated by every handoff — watching it would make each
	// auto-handoff immediately trigger the next one, forever.
	if name == orchestrator.MirrorFileName {
		return false
	}
...
func (w *Watcher) triggerHandoff(absPath string, log *slog.Logger) {
	...
	if w.cfg.Notify {
		sent := notify.Send("AgentOrchestra: งานถึงคิว "+env.TargetAgent,
			fmt.Sprintf("%s เปลี่ยน — %s", rel, env.Payload.Task))
		log.Debug("watch: desktop notification", "sent", sent)
	}
}
